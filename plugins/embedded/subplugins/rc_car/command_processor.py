"""
BCI Command Processor
=====================
Filters, debounces, logs, and maps EMOTIV EPOC X mental command streams into
safe, actionable motor commands for the ESP32 RC Car.
"""
...
class CommandProcessor:
    """
    Processes raw BCI stream inputs with safety thresholds, debouncing, duplicate filtering,
    auto-stop timeouts, and CSV telemetry logging.
    """
    ...
    def _init_csv_log(self):
        """
        Ensures CSV telemetry log exists with proper column headers.
        """
        try:
    ...
    def log_telemetry(self,
                      ...
                      error_msg: str = "None"):
        """
        Appends a single telemetry record to the CSV log file.
        """
    ...
    def process_command(self, raw_command: str, confidence: float) -> Tuple[bool, str, str]:
        """
        Evaluates an incoming BCI command frame against safety rules and maps to external payload.
        ...
        if confidence < self.confidence_threshold:
            ...
            print(f"[3] COMMAND VALIDATED: False (Low Confidence)")
        ...
        print(f"[3] COMMAND VALIDATED: True")
        logger.info(f"ACCEPTED BCI COMMAND: '{command}' (conf={confidence:.2f}) -> MQTT Payload: '{external_command}'")
    ...
    def check_watchdog_timeout(self) -> bool:
        ...
        if self.active_state != "STOP":
            if (current_time - self.last_command_time) > config.SAFETY_AUTO_STOP_TIMEOUT:
                ...
                stop_payload = config.COMMAND_MAP.get("STOP", "LIFTCARSTOP")
                if self.on_action_ready:
