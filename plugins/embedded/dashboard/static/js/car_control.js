...
// Detect touch devices (mobile/tablet). On touch devices we disable keyboard controls.
const isTouchDevice = ('ontouchstart' in window) || navigator.maxTouchPoints > 0;
state.keyboardEnabled = !isTouchDevice;
...
function setupSimulationFullscreen(card, viewport, button) {
    ...
    const fullscreenExitButton = el("fullscreenExitBtn");
    const mobileFullscreenEnterButton = el("mobileFullscreenEnterBtn");

    const openFullscreen = async () => {
        if (document.fullscreenElement === card) return true;
        if (!card.requestFullscreen || document.fullscreenEnabled === false) {
        ...
        try {
            ...
            console.warn("[3D] Fullscreen request failed.", error);
    ...
    const closeFullscreen = async () => {
        ...
        try {
            ...
            console.warn("[3D] Fullscreen exit failed.", error);
            notify("Fullscreen could not be closed.");
            return false;
        }
    };
    ...
    mobileFullscreenEnterButton?.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        openFullscreen();
    });
    ...
    const updateFullscreenState = () => {
        ...
        /* Static fullscreen controls are the reliable user-facing controls. */
        if (fullscreenExitButton) {
            fullscreenExitButton.hidden = !active;
        }

        if (mobileFullscreenEnterButton) {
            mobileFullscreenEnterButton.hidden = active;
        }
...
async function loadRobotModel() {
    ...
    try {
        ...
        /* Normalize model size/origin so any valid GLB is visible in the TPP camera. */
        model.updateMatrixWorld(true);
        const bounds = new THREE.Box3().setFromObject(model);
        const boundsSize = new THREE.Vector3();
        bounds.getSize(boundsSize);
        const maxDimension = Math.max(boundsSize.x, boundsSize.y, boundsSize.z);

        if (Number.isFinite(maxDimension) && maxDimension > 0.0001) {
            model.scale.multiplyScalar(3.4 / maxDimension);
            model.updateMatrixWorld(true);
...
function animate(timestamp) {
    ...
    /*
     * Use the requestAnimationFrame timestamp directly. Clamp the
     * step so returning from a background/suspended tab cannot cause
     * the vehicle to jump.
     */
    ...
    if (lastAnimationTimestamp !== null) {
        delta = Math.min(
            Math.max(
                (currentTimestamp - lastAnimationTimestamp) / 1000,
                0
            ),
            0.05
        );
...
/* Touch devices use the on-screen controls; hide keyboard UI. */
const keyboardBadge = el("keyboardDriveBadge");
if (keyboardBadge) {
    keyboardBadge.hidden = isTouchDevice;
}
if (keyboardToggle) {
    keyboardToggle.hidden = isTouchDevice;
}
    ...
    websocket.on(
        ...
        (data) => {
            ...
            if (
                ...
                data.device &&
                data.device !== state.selectedDevice &&
                state.selectedDevice !== "98:A3:16:BF:2C:C0"
            ) {
                return;
            }
            ...
            if (data.battery_soc_pct !== undefined && data.battery_soc_pct !== null) {
                ...
                const batteryEl = el("carBatterySoc");
                const hudBatteryEl = el("hudBatterySoc");
                const progressEl = el("batteryProgressBar");
                const chargeLabel = el("batteryChargeLabel");
                const badgeEl = el("batteryStatusBadge");

                if (batteryEl) batteryEl.textContent = `${pct}%`;
                if (hudBatteryEl) {
                    hudBatteryEl.textContent = `${pct}%`;
                    if (pct <= 20) hudBatteryEl.style.color = "#ef4444";
                    else if (pct <= 50) hudBatteryEl.style.color = "#f59e0b";
                    else hudBatteryEl.style.color = "#00f6ff";
                }
                ...
                if (chargeLabel) {
                    if (pct > 75) chargeLabel.textContent = "OPTIMAL";
                    else if (pct > 35) chargeLabel.textContent = "NORMAL";
                    else if (pct > 15) chargeLabel.textContent = "LOW";
                    else chargeLabel.textContent = "CRITICAL";
                }

                if (badgeEl) {
                    const color = pct > 20 ? "#10b981" : "#ef4444";
                    badgeEl.innerHTML = `<span style="background:${color};box-shadow:0 0 8px ${color};"></span> ${pct}%`;
                }
            }

            // Update Supply Voltage
            if (data.supply_voltage_v !== undefined && data.supply_voltage_v !== null) {
                const voltEl = el("carSupplyVoltage");
                if (voltEl) voltEl.textContent = `${data.supply_voltage_v} V`;
            }

            // Update Movement State
            if (data.state) {
                const visualState = el("visualState");
                if (visualState) visualState.textContent = data.state;
            }

            // Update Obstacles if present
            if (data.front_obstacle !== undefined) {
                setObstacleDirection("front", Boolean(data.front_obstacle));
            }
            if (data.rear_obstacle !== undefined) {
                setObstacleDirection("rear", Boolean(data.rear_obstacle));
            }
        }
    );
}
